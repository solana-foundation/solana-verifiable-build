...
RUN curl -sSfL "https://release.anza.xyz/v4.0.1/install" -o /tmp/solana_install.sh && \
    ACTUAL=$(sha256sum /tmp/solana_install.sh | awk '{print $1}') && \
    test "$ACTUAL" = "954aa3c2753d1b55cc2492708ec29bdf067ecab39710cd2e1771b72f125ee701" && \
